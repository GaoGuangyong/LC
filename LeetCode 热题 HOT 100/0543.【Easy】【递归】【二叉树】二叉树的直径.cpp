...
public:
    ...
    int dfs(TreeNode* root) {
        ...
        // 递归左右子树
        int left = dfs(root->left); // 求当前节点左子树的深度
        int right = dfs(root->right); // 求当前节点右子树的深度

        // 以当前节点为根的子树的直径 = 左子树的深度 + 右子树的深度
        res = max(res, left + right); // 更新直径的最大值

        // 返回以该节点为根的子树的深度
        return max(left, right) + 1; // 深度 = max(左儿子深度，右儿子深度) + 当前节点指向这两个儿子的边
    }
