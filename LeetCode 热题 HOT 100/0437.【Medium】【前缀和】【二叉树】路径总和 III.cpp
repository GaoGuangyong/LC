...
class Solution {
public:
    unordered_map<long long, int> hash; // 定义哈希表，key:前缀和，value:前缀和出现的次数
    int res = 0;

    // 递归函数：从节点 root 开始遍历，目标和为 target，到当前节点之前的前缀和为 curSum
    void dfs(TreeNode* root, int target, long long curSum) {
        // 递归出口：如果遍历到了空节点，则直接返回
        if (root == nullptr) return;

        curSum += root->val; // 前缀和 += 当前节点的值
        res += hash[curSum - target]; // 将前缀和 curSum - target 出现的次数加到结果上
