...
class Solution {
public:
    unordered_map<int, int> hash; // 定义一个哈希表，存中序遍历序列中每个元素的下标
    vector<int> preorder, inorder; // 为了在 dfs 函数里使用 preorder, inorder，所以声明成全局变量

    // DFS 函数：根据 preorder[pl ~ pr] 和 inorder[il ~ ir] 构造二叉树，返回根节点
    TreeNode* dfs(int pl, int pr, int il, int ir) {
        if (il > ir) return nullptr; // 如果当前区间长度为 0，返回空
