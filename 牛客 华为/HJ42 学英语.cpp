...
vector<int> ihundreds{ (int)1e2, (int)1e3, (int)1e6, (int)1e9};
...
string itoe(long long n) {
    ...
    if (20 <= n && n < 1e2) {
        ...
        // 如果个位为 0
            ...
            return twenties[n / 10];
    }
    // > 100 每三位数后记得带上计数单位 分别是 thousand, million, billion
    for (int i = 0; i < 4; i ++ ) { 
        if (n < ihundreds[i + 1]) { // 确定范围
            // 如果 n 不是 整百、整千、整百万、整十亿
            if (n % ihundreds[i] != 0) {
                // 如果 i = 1, 2, 3，则不是三位数，thousand、million、billion 后不加 and
                if (i != 0) {
                    return itoe(n / ihundreds[i]) + " " + hundreds[i] + " " + itoe(n % ihundreds[i]);
                } 
                // 如果 i = 0，则是三位数，hundred 后加 and
                else {
                    return itoe(n / ihundreds[i]) + " " + hundreds[i] + " and " + itoe(n % ihundreds[i]);
                }
            } 
            // 如果 n 是 整百、整千、整百万、整十亿
            else {
