...
// 解法二：DP （推荐）
// f[i][j] 表示把 i 个苹果放到 j 个盘子里的方案数
// 例如：m = 5,  n = 2，则有 {0, 5} {1, 4} {2, 3} 共 3 种方法，即 f[5][2] = 3
// {0, 5} 这种方案即为 f[5][1]
...
int main() {
    ...
    for (int i = 1; i <= m; i ++ ) { 
        for (int j = 1; j <= n; j++) {
            ...
            if (i < j) f[i][j] = f[i][i];
            // 如果苹果数量 >= 盘子的数量，则等于 空一个盘子的方案数（f[i][j - 1]）+ 一个盘子都不空的方案数
            // 一个盘子都不空，即先把 j 个苹果放到 j 个盘子里，每个盘子放一个，再把剩下的 i - j 个苹果放到 j 个盘子里
