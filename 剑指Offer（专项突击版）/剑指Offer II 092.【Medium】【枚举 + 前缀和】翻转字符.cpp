...
// 从 1 到 n 枚举，对于每个当前遍历的位置 i：设这个位置是最终第一个 1 的位置
...
// 设前缀和数组是 f，则 s[j ~ i] 内的元素的和是 f[i] - f[j - 1]
...
public:
    int minFlipsMonoIncr(string s) {
        ...
        // 初始化前缀和数组
        for (int i = 1; i <= n; i ++ ) {
            f[i] = f[i - 1] + (s[i - 1] - '0'); // 将 char 型数字转化为 int 型数字
        }

        // 关键！！！因为求最少翻转次数，故初始时，给定一个最大翻转次数（s 全是 1，都变成 0 需要转换的次数就是 1 的个数 f[n]）
        int res = f[n];
