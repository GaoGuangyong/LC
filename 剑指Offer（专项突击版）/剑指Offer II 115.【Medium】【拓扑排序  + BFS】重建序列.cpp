...
public:
    bool sequenceReconstruction(vector<int>& nums, vector<vector<int>>& sequences) {
        ...
        // 构建邻接表：将所有的边插入邻接表，graph[i] 存 i 的所有后继
        for (auto& e: sequences) {
            // 边数组 e 的长度可能 ≥ 2，因此需要俩俩一组进行枚举
        ...
        while (q.size()) { // 只要队列不空，就不断将节点入队
            ...
            // 只有队列的大小是 1，才取出队头节点（唯一的入度为 0 的点）
            auto t = q.front();
            q.pop();
            
            res.push_back(t + 1); // 将队头节点插入 res 中，注意要 + 1（因为建图的时候 - 1 了）

            // 枚举当前节点的所有后继节点，将每条边删去，即后继节点的入度 -1，如果入度降为 0，再加入队列
            for (int i: graph[t]) {
                d[i] --; // 将后继节点 i 的入度 -1
                if (d[i] == 0) q.push(i); // 如果入度降为 0，再加入队列
            }
        }
