// 滑动窗口
// 使用两个哈希表 hash_p, hash 来分别记录字符串 p 中每个字母的个数 和 滑动窗口中每个字母的个数
// 先记录字符串 p 中每个字母的个数
...
public:
    ...
    unordered_map<char, int> hash; // 滑动窗口 中每个字母及其个数

    // 判断是否匹配，即：判断滑动窗口中的字母 c 是否在 p 中存在，且个数相同
    bool check(char c) {
        if (hash_p.count(c) && hash_p[c] == hash[c]) {
            return true;
        }
        return false;
    }
    ...
    vector<int> findAnagrams(string s, string p) {
        ...
        for (int i = 0, j = 0; i < s.size(); i ++ ) {
            ...
            hash[s[i]] ++ ; // 将当前字母 s[i] 加入滑动串口（滑动窗口中，s[i] 的个数 + 1）
            ...
            while (i - j + 1 > p.size()) {
                ...
                hash[s[j]] -- ; // 将字母 s[j] 从滑动串口中删除（滑动窗口中，s[j] 的个数 - 1）
...
public:
    ...
    bool check(char c) {
        return hash_p.count(c) && hash_p[c] == hash[c];
    }
