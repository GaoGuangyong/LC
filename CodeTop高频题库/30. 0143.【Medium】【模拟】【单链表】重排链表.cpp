...
// 然后搞两个指针，一个指向前一半的起点，一个指向后一半（翻转后）的起点，只要把后面一半的每个节点插入到前面一半的节点和节点之间就可以了
...
public:
    void reorderList(ListNode* head) {
        ...
        // 反转链表的后半部分 [mid ~ n] 共 (n / 2) + 1 个节点，共反转 n / 2 条边
        auto a = mid;
        auto b = a->next;
        ...
        // 将翻转后的部分按顺序一个一个插入到左边的链表的每两个节点之间
        auto l = head; // 指针 l 指向的前半部分的起点
        auto r = a;  // 指针 r 指向的后半部分的起点

        // 枚举前一半节点（头节点 ~ mid 前一个节点）
        for (int i = 0; i < n / 2; i ++ ) {
            // 先用指针 t 记录下 r 的后继
            auto t = r->next;
