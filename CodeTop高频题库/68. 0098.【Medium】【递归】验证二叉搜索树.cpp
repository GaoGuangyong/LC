...
public:
    ...
    bool dfs(TreeNode* root, LL min, LL max) { // 注意：本题数据可能越界，将返回设置成 LL
        // 递归出口：
        if (root == nullptr) return true; // 如果遍历到空节点，则遍历结束，说明是二叉搜索树
        if (root->val <= min || root->val >= max) return false; // 如果当前遍历节点的值在合法区间外，则不是二叉搜索树

        // 否则，递归遍历左右子树
        bool l = dfs(root->left, min, root->val); // 判断 root 的左儿子的值是否在 (min, root->val) 范围
        bool r = dfs(root->right, root->val, max); // 判断 root 的右儿子的值是否在 (root->val, max) 范围

        return l == true && r == true; // 只有左子树 和 右子树都是，root 才是
    }

    // 主函数：判断以 root 为根节点的树是否为二叉搜索树
